...
namespace RH_TPA.Receptionist
{
    public partial class SalesReportForm : Form
    {
        ...
        {
            var result = (from x in db.SalesReport
                          where x.Deleted == 0
                          select new
                          {
                              SalesReportId = x.SalesReportId,
                              TotalIncome = x.TotalPayment
                          }).ToList();
            return result;
        }
        ...
        {
            int total = (from DataGridViewRow x in dataGridView1.Rows select Convert.ToInt32(x.Cells[1].FormattedValue)).Sum();
            ...
            for (int i = 0; i < dataGridView1.Rows.Count; i++)
            {
                ...
                var obj = (from x in db.SalesReport where x.SalesReportId == id select x).FirstOrDefault();
                obj.Deleted = 1;
                db.SaveChanges();
            }
            worksheet.Cells[dataGridView1.Rows.Count + 2, 1] = "Total Income " + arrMonth[month - 1] + " : Rp." + total;

            var saveFile = new SaveFileDialog();
            saveFile.FileName = "SalesReport" +arrMonth[month];
